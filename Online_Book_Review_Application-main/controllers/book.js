...
export async function getBooksByISBN(req, res) {
    try {
        ...
        if (!isbn) {
            return res.json({ message: "Enter a valid ISBN Code!" });
        }
        ...
        res.json({ message: "No book found with the given ISBN Code!" });
    } catch (error) {
...
export async function getBooksByAuthor(req, res) {
    try {
        ...
        if (!author) {
            return res.json({ message: "Provide a valid author name!" });
        }
